...
describe('FlashTake', function() {
  const tradeAssets1 = [
    {
      ...assets[0],
      positionFeeRate: new BigNumber('0.001'),
      longCumulativeFundingRate: new BigNumber('0.0009') // 0.0003 * 3
    },
    assets[1]
  ]
  const tradeAssets2 = [
    {
      ...assets[0],
      positionFeeRate: new BigNumber('0.001'),
      longCumulativeFundingRate: new BigNumber('0.0018') // 0.0009 + 0.0003 * 3
    },
    assets[1]
  ]
  const tradePrices1 = {
    ETH: new BigNumber('2000'),
    USDC: _1
  }
  const subAccount1 = {
    ...accountStorage0.subAccount,
    collateral: new BigNumber('10')
  }
  const subAccountId = encodeSubAccountId('0x1111111111111111111111111111111111111111', 0, 0, true)

  it('normal', () => {
    const result1 = computeOpenPosition(
      tradeAssets1,
      subAccountId,
      subAccount1,
      tradePrices1,
    ...
    expect(result1.feeUsd).toApproximate(new BigNumber('2')) // 0.001
    expect(result1.afterTrade.subAccount.collateral).toApproximate(new BigNumber('9.989'))
    expect(result1.isTradeSafe).toEqual(true)
    const tradePrices2 = {
      ETH: new BigNumber('2100'), // pnl = $100 = 0.047619047619047619
      USDC: _1
    }
    const result2 = computeClosePosition(
      tradeAssets2,
      subAccountId,
      result1.afterTrade.subAccount,
      0,
      tradePrices2,
      new BigNumber('1'),
      new BigNumber('0.01')
    )
    expect(result2.feeUsd).toApproximate(new BigNumber('3.99')) // funding fee = 0.0009, pos fee = 0.001
    expect(result2.afterTrade.subAccount.collateral).toApproximate(new BigNumber('9.979')) // 9.989 - gasFee - withdraw
    expect(result2.isTradeSafe).toEqual(true)
  })

  it('gas + fee > collateral > gas', () => {
    const result1 = computeOpenPosition(
      tradeAssets1,
      subAccountId,
      subAccount1,
    ...
    expect(result1.afterTrade.subAccount.collateral).toApproximate(new BigNumber('9.999'))
    const tradePrices2 = {
      ETH: new BigNumber('190.519737844840725499'), // pnl = -$1809.471187280296462833 = -9.4976
      USDC: _1
    }
    const result2 = computeClosePosition(
      tradeAssets2,
    ...
    expect(result2.feeUsd).toApproximate(new BigNumber('0.26672763298277701570')) // gasFee = 0.5, funding fee = 0.0009, pos fee(reduced) = 0.0005, remainingCollateral = 0, wallet balance not changed
    expect(result2.afterTrade.subAccount.collateral).toApproximate(new BigNumber('0'))
    expect(result2.isTradeSafe).toEqual(true)
  })
  ...
  it('collateral < gas', () => {
    ...
    expect(result1.afterTrade.subAccount.collateral).toApproximate(new BigNumber('9.999'))
    const tradePrices2 = {
      ETH: new BigNumber('183.503073676484081108'), // pnl = -$1816.47335193070033769 = -9.899
      USDC: _1
    }
    const result2 = computeClosePosition(
      tradeAssets2,
    ...
    expect(result2.feeUsd).toApproximate(new BigNumber('0')) // gasFee(reduced) = 0.1, funding fee(reduced) = 0, pos fee(reduced) = 0, remainingCollateral = 0, wallet balance not changed
    expect(result2.afterTrade.subAccount.collateral).toApproximate(new BigNumber('0'))
    expect(result2.isTradeSafe).toEqual(true)
  })
  ...
  it('bankrupt', () => {
    ...
    expect(result1.afterTrade.subAccount.collateral).toApproximate(new BigNumber('9.999'))
    const tradePrices2 = {
      ETH: new BigNumber('181.833059068469238392'), // pnl = -$1818.166940931530761608 = -9.9991
      USDC: _1
    }
    expect((): void => {
      computeClosePosition(
